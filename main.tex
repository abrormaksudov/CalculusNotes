...
\begin{tcolorbox}
    Let $f$ and $g$ be differentiable functions. Then the following rules hold:
    \[
    \begin{aligned}
...
\begin{tcolorbox}
    ...
    \begin{itemize}
        ...
        \item \textbf{Local Maximum:} $f(c)$ is a local maximum if $\exists \delta > 0$ such that $f(c) \geq f(x),\\ \ \forall x \in (c - \delta, c + \delta).$
        \item \textbf{Local Minimum:} $f(c)$ is a local minimum if $\exists \delta > 0$ such that $f(c) \leq f(x),\\ \ \forall x \in (c - \delta, c + \delta).$
    \end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}
    ...
    \[ \exists c,d \in [a,b] \text{ such that } f(c) \leq f(x) \leq f(d),\ \forall x \in [a,b]. \]
\end{tcolorbox}
...
\begin{tcolorbox}
    ...
    \begin{itemize}
        \item If $f''(x) > 0\ \forall x \in I \implies f(x)$ is \textbf{concave up} on $I$.
        \item If $f''(x) < 0\ \forall x \in I \implies f(x)$ is \textbf{concave down} on $I$.
    \end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}
    ...
    \]
    where the subintervals $[x_{i-1}, x_i]$ have length $\Delta x_i = x_i - x_{i-1}$. For each subinterval $[x_{i-1}, x_i]$, let $x_i^*$  be an \textbf{arbitrary sample point} within the interval, $x_i^* \in [x_{i-1}, x_i]$. Then, a \textbf{Riemann sum}, which \textbf{approximates} the area under the curve $f(x)$ over $[a,b]$, is defined as:
    \[
...
\begin{tcolorbox}[breakable]
    ...
    \begin{enumerate}
        ...
        \[
        S_{\text{right}} = \sum_{i=1}^n f(x_{i-1})\Delta x_i    
        \]
